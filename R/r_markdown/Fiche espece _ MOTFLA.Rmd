...
# Fiche espèce : MOTFLA (Bergeronnette printanière)
...
2)  Pré-traitement de MOTFLA_data

```{r}

MOTFLA_data <- subset(PE, CODE == "MOTFLA")
#### ENLEVER LES SITES VIDES
occ_site <- tapply(MOTFLA_data$ABONDANCE, MOTFLA_data$SITE, function(x) sum(x > 0))
hist(tapply(MOTFLA_data$ABONDANCE, MOTFLA_data$SITE, function(x) sum(x > 0))) 
site_vide <- names(occ_site[occ_site < 1])
site_vide#liste des sites avec 0 occurence de MOTFLA 
MOTFLA_data2<- MOTFLA_data[!(MOTFLA_data$SITE %in% site_vide),]
# #### ENLEVER LES ANNEES VIDES
# occ_year <- tapply(MOTFLA_data$ABONDANCE, MOTFLA_data$ANNEE_txt, function(x) sum(x > 0))
# occ_year
# # Filtrer le jeu de données pour exclure annees de debut vide
# MOTFLA_data3<- MOTFLA_data2[!(MOTFLA_data2$SITE %in% YEAR_vide),]
# ifelse(occ_year[2002]== 0, MOTFLA_data2 == MOTFLA_data2[!(MOTFLA_data2$ANNEE_txt == "2002"),])
...
md_va_MOTFLA <- glmmTMB(ABONDANCE ~ ANNEE_txt + (1|SITE) , data = MOTFLA_data2, family = nbinom2, ziformula = ~1)
summary(md_va_MOTFLA)


#Residus 
library(DHARMa)
simulationOutput <- simulateResiduals(fittedModel = md_va_MOTFLA, plot = T)#met du temps 
testZeroInflation(simulationOutput)


# Graphique d'abondance en fonction de l'année 
gg2<- ggpredict(md_va_MOTFLA,terms = c("ANNEE_txt"))
...
md_td_MOTFLA <- glmmTMB(ABONDANCE ~ annee_sc + (1|SITE) , data = MOTFLA_data, family = nbinom2, ziformula = ~1)
summary(md_td_MOTFLA)# resume du modèle


#Residus 
simulationOutput <- simulateResiduals(fittedModel = md_td_MOTFLA, plot = T)#met du temps 
testZeroInflation(simulationOutput)
res5<-residuals(md_td_MOTFLA)
...
confint(md_td_MOTFLA)
gg3<- ggpredict(md_td_MOTFLA,terms = c("annee_sc"))
...
```{r hab}
length(unique(MOTFLA_data2$SITE)) #Nombre de sites où elle a été vu sum(MOTFLA_data2$ABONDANCE)
#Nombre d'individu de l'espece entendu au total :

list_x <- as.factor(c(2002:2021))

for (x in list_x) { MOTFLA_data_new <-
MOTFLA_data2[(MOTFLA_data2$ANNEE_txt == x),] 
ar <-tapply(MOTFLA_data_new$ABONDANCE, MOTFLA_data_new$SITE, function(a)
sum(a >= 1))

ar[is.na(ar)] <- 0
presence <- names(ar[ar > 0]) 
nb<- length(presence) 
nb_indiv <- sum(MOTFLA_data_new$ABONDANCE) 
...
Résultat non significatif, les abondances de bergeronnettes sont
globalement faibles et surtout fluctuante en fonction du temps. (une
vingtaine d'individus par an)

Biblio : Livre oiseaux d'europe : prés humides et bord de marais,
insectivore
