...
# Fiche espèce : PARMAJ (Bergeronnette printanière)
...
2)  Pré-traitement de PARMAJ_data 

```{r}

PARMAJ_data <- subset(PE, CODE == "PARMAJ")
#### ENLEVER LES SITES VIDES
occ_site <- tapply(PARMAJ_data$ABONDANCE, PARMAJ_data$SITE, function(x) sum(x > 0))
hist(tapply(PARMAJ_data$ABONDANCE, PARMAJ_data$SITE, function(x) sum(x > 0))) 
site_vide <- names(occ_site[occ_site < 1])
site_vide#liste des sites avec 0 occurence de PARMAJ 
PARMAJ_data2<- PARMAJ_data[!(PARMAJ_data$SITE %in% site_vide),]
# #### ENLEVER LES ANNEES VIDES
# occ_year <- tapply(PARMAJ_data$ABONDANCE, PARMAJ_data$ANNEE_txt, function(x) sum(x > 0))
# occ_year
# # Filtrer le jeu de données pour exclure annees de debut vide
# PARMAJ_data3<- PARMAJ_data2[!(PARMAJ_data2$SITE %in% YEAR_vide),]
# ifelse(occ_year[2002]== 0, PARMAJ_data2 == PARMAJ_data2[!(PARMAJ_data2$ANNEE_txt == "2002"),])
...
md_va_PARMAJ <- glmmTMB(ABONDANCE ~ ANNEE_txt + (1|SITE) , data = PARMAJ_data2, family = nbinom2, ziformula = ~1)
summary(md_va_PARMAJ)


#Residus 
library(DHARMa)
simulationOutput <- simulateResiduals(fittedModel = md_va_PARMAJ, plot = T)#met du temps 
testZeroInflation(simulationOutput)


# Graphique d'abondance en fonction de l'année 
gg2<- ggpredict(md_va_PARMAJ,terms = c("ANNEE_txt"))
...
md_td_PARMAJ <- glmmTMB(ABONDANCE ~ annee_sc + (1|SITE) , data = PARMAJ_data, family = nbinom2, ziformula = ~1)
summary(md_td_PARMAJ)# resume du modèle


#Residus 
simulationOutput <- simulateResiduals(fittedModel = md_td_PARMAJ, plot = T)#met du temps 
testZeroInflation(simulationOutput)
res5<-residuals(md_td_PARMAJ)
...
confint(md_td_PARMAJ)
gg3<- ggpredict(md_td_PARMAJ,terms = c("annee_sc"))
...
```{r hab}
length(unique(PARMAJ_data2$SITE)) #Nombre de sites où elle a été vu sum(PARMAJ_data2$ABONDANCE)
#Nombre d'individu de l'espece entendu au total :

list_x <- as.factor(c(2002:2021))

for (x in list_x) { PARMAJ_data_new <-
PARMAJ_data2[(PARMAJ_data2$ANNEE_txt == x),] 
ar <-tapply(PARMAJ_data_new$ABONDANCE, PARMAJ_data_new$SITE, function(a)
sum(a >= 1))

ar[is.na(ar)] <- 0
presence <- names(ar[ar > 0]) 
nb<- length(presence) 
nb_indiv <- sum(PARMAJ_data_new$ABONDANCE) 
...
Résultat non significatif, les abondances de mésanges charbo semblent être stable ? 
L'année n'a pas d'influence sur l'abondance de mésange charbonière

Biblio : 

Rapport evaluation oiseaux nicheurs UICN : espèce en augmentation présente sur tout le territoire, pas de menace identifiée. 

résultat en contradiction avec les données france ? augmentation VS stable 